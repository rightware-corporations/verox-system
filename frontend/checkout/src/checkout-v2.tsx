...
const providers: Provider[] = [
  ...
  { id: 'EMOLA', name: 'e-Mola', kind: 'Mobile money', monogram: 'e', enabled: false, recipient: '+258 87 557 9796', recipientName: 'Owen de Jesus', status: 'A aguardar servidor' },
  { id: 'MILLENNIUM_BIM', name: 'Millennium bim', kind: 'Conta bancária', monogram: 'bim', enabled: false, status: 'Configuração pendente' },
];
...
function AppChrome({ children, screen }: { children: React.ReactNode; screen: Screen }) {
  ...
  return <main className="v2-page"><div className="v2-atmosphere" aria-hidden="true"><i/><i/><i/><i/></div><section className="v2-shell">
    <header className="v2-header"><Brand/><div className="v2-secure"><LockKeyhole size={13}/><span>Checkout seguro</span></div></header>
    ...
    <footer className="v2-footer"><ShieldCheck size={14}/><span>Verificação protegida por</span><Brand quiet/></footer>
  </section></main>;
}

function TransactionHero() {
  return <section className="v2-hero"><div className="v2-hero-meta"><span>Pagamento para</span><strong>{payment.reference}</strong></div><h1>{payment.merchant}</h1><div className="v2-money"><strong>1 500,00</strong><span>MZN</span></div><p>Confirme o método e conclua o pagamento no seu canal habitual.</p></section>;
}

function ProviderMark({ provider }: { provider: Provider }) {
  return <span className={`v2-provider-mark ${provider.id.toLowerCase()}`}>{provider.id==='MILLENNIUM_BIM'?<Landmark size={18}/>:provider.monogram}</span>;
}
...
case'verifying':content=<StateScreen tone="info" icon={<VerificationLoader/>} title="Verificação em curso" headline="Estamos a verificar o seu pagamento" body="Recebemos a confirmação e estamos a validar a transacção."><div className="v2-state-callout info"><Clock3 size={17}/><span>Não precisa de enviar a mensagem novamente. O pagamento ainda não está confirmado.</span></div></StateScreen>;break;
...
case'review':content=<StateScreen tone="warning" icon={<TriangleAlert/>} title="Revisão necessária" headline="Estamos a analisar o seu pagamento" body="Recebemos a informação, mas ainda não é seguro declarar o pagamento confirmado."><div className="v2-state-callout warning"><TriangleAlert size={17}/><strong>Não efectue outro pagamento.</strong><span>A transacção pode já ter sido efectuada.</span></div></StateScreen>;break;
