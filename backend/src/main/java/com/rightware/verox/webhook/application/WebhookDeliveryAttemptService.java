...
public class WebhookDeliveryAttemptService {
    ...
    public void attempt(UUID deliveryId) {
        ...
        String endpointPublicId = delivery.getEndpoint().getPublicId();
        String signature = signatureService.signatureHeader(endpointPublicId, payload, attemptedAt);

        Map<String, String> headers = new LinkedHashMap<>();
        headers.put("VEROX-Event-Id", delivery.getEvent().getPublicId());
        headers.put("VEROX-Event-Type", delivery.getEvent().getType());
    ...
    private void recordFailure(WebhookDelivery delivery, Integer statusCode, String error, Instant attemptedAt) {
        Instant retryAt = attemptedAt.plusSeconds(retryDelaySeconds(delivery.getAttemptCount()));
        delivery.recordFailure(statusCode, error, attemptedAt, retryAt, maxAttempts);
        deliveryRepository.save(delivery);
        log.warn(
    ...
    private long retryDelaySeconds(int previousAttempts) {
        int exponent = Math.min(Math.max(previousAttempts, 0), 20);
        long multiplier = 1L << exponent;
        if (baseRetrySeconds > maxRetrySeconds / multiplier) {
            return maxRetrySeconds;
        }
        return Math.min(maxRetrySeconds, baseRetrySeconds * multiplier);
    }
